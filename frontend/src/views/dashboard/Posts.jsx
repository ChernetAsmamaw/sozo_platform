...
function Posts() {
  ...
  const fetchPosts = async () => {
    const post_res = await apiInstance.get(
      `/author/dashboard/list-post/${userId}`
    );
  ...
  // Search for post by the logged in user
  const handleSearch = (e) => {
    ...
    if (query === "") {
      ...
      const filtered = posts.filter((p) => {
        // Check if the title includes any part of the query
        return p.title.toLowerCase().includes(query);
      });

      // Change the state to only show the filtered posts
      setPosts(filtered);
    }
  };
  ...
  const handleSortChange = (e) => {
    ...
    if (sortValue === "Newest") {
      // Use the sort method to sort the posts by date
      // Get newest date by subtracting the older date from the newer date
